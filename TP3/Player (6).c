...
void troca(CelulaDupla *a, CelulaDupla *b) {
    Player temp = a->elemento;
    a->elemento = b->elemento;
    b->elemento = temp;
}

CelulaDupla *particiona(CelulaDupla *inicio, CelulaDupla *fim) {
    Player pivo = fim->elemento;
    CelulaDupla *i = inicio->ant;

    for (CelulaDupla *j = inicio; j != fim; j = j->prox) {
        // Compara birthState
        int comp = strcmp(j->elemento.birthState, pivo.birthState);
        
        if (comp < 0 || (comp == 0 && strcmp(j->elemento.name, pivo.name) <= 0)) {
...
void quicksort(CelulaDupla *inicio, CelulaDupla *fim) {
    if (fim != NULL && inicio != fim && inicio != fim->prox) {
        CelulaDupla *pivo = particiona(inicio, fim);
