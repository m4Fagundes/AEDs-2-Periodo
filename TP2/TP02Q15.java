...
public class TP02Q15 {
    public static void main(String[] args) {
        ...
        int k = scanner.nextInt(); //quantidade da entrada parcial
        scanner.nextLine();

        quantidadeEntrada = leitura(players, playesSelessao, scanner);

        // algoritmo de ordenacao parcial por selecao 
        

        selecaoParcial(playesSelessao, quantidadeEntrada,k);
    ...
    public static void selecaoParcial(Jogador[] players, int contador, int k){
        k=10;
        for(int i = 0; i < k; i++){
            int menor = i;
            for(int j = (i+1); j < contador; j++){
                if(players[menor].getNome().compareTo(players[j].getNome()) > 0){
                    menor = j;
                }
            }
            swap(players, menor, i);
        }
    }
