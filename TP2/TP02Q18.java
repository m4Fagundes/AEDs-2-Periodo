...
public class TP02Q18 {
    public static void main(String[] args) {
        ...
        int k = scanner.nextInt();// quantidade de entradas parciais
        scanner.nextLine();

        int quantidadeEntrada = leitura(players, playesSelessao, scanner);

        quicksortParcial(playesSelessao, 0, quantidadeEntrada - 1, k);
    ...
    public static void quicksortParcial(Jogador[] players, int esq, int dir, int k) {
        ...
        int j = separar(players, esq, dir);
        if (j > k) {
            quicksortParcial(players, esq, j - 1, k);
        } else if (j < k) {
            quicksortParcial(players, j + 1, dir, k);
        }
    }

    public static int separar(Jogador[] players, int esq, int dir) {
        int i = esq + 1;
        int j = dir;
        Jogador pivo = players[esq];
        while (i <= j) {
            if (players[i].getEstadoNascimento().compareTo(pivo.getEstadoNascimento()) < 0) {
                i++;
            } else if (players[j].getEstadoNascimento().compareTo(pivo.getEstadoNascimento()) > 0) {
                j--;
            } else if (players[i].getEstadoNascimento().compareTo(pivo.getEstadoNascimento()) == 0) {
                if (players[i].getNome().compareTo(pivo.getNome()) < 0) {
                    i++;
                } else if (players[j].getNome().compareTo(pivo.getNome()) > 0) {
                    j--;
                } else {
                    swap(players, i, j);
                }
    ...
    public static void countingSort(Jogador[] players, int contador) {
        // Encontre o valor máximo da altura para determinar o tamanho do array
        // auxiliar.
        int maior = 0;
        for (int i = 0; i < contador; i++) {
        ...
        while (!id.equals("FIM")) {
            try {
                ...
                if (players[playerIndex] != null) {
                    playersSelecao[i] = new Jogador(players[playerIndex].getId(), players[playerIndex].getNome(),
                            players[playerIndex].getAltura(), players[playerIndex].getPeso(),
                            players[playerIndex].getUniversidade(), players[playerIndex].getAnoNascimento(),
                            players[playerIndex].getCidadeNascimento(), players[playerIndex].getEstadoNascimento());
                    i++;
                    contador++;
                } else {
    ...
    public static void preencheJogador(Jogador[] players) {
        // String csvArquivo = "C:\\Users\\mathe\\OneDrive\\Área de
        // Trabalho\\players_atualizado.csv";
        ...
        try {
            ...
            while ((linha = conteudoCSV.readLine()) != null) {
                ...
                if (campos[0].equals("id"))
                    continue; // Pula a primeira linha do CSV (cabeçalho
    ...
    public static void AtualizaCampoVazio() {
        // String csvArquivo = "C:\\Users\\mathe\\OneDrive\\Área de
        // Trabalho\\players.csv";
        // String novoCsvArquivo = "C:\\Users\\mathe\\OneDrive\\Área de
        // Trabalho\\players_atualizado.csv";
        String csvArquivo = "/tmp/players.csv";
        ...
        try (BufferedReader conteudoCSV = new BufferedReader(new FileReader(csvArquivo));
            ...
            while ((linha = conteudoCSV.readLine()) != null) {
                ...
                for (int i = 0; i < campos.length; i++) {
                    if (campos[i].trim().isEmpty()) { // Use trim() para remover espaços em branco antes de verificar se
                                                      // está vazio
