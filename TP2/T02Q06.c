...
void abreFechaLeEAtualizaArquivo()
{
    ...
    FILE *temp = fopen("/tmp/temp.csv", "w");
    if (temp == NULL)
    {
    ...
    remove("/tmp/players.csv");
    rename("/tmp/temp.csv", "/tmp/players.csv");
}
...
void imprimir(Jogador *players, int contador) {
    for (int i = 0; i < contador; i++) {
        printf("[%d ## %s ## %d ## %d ## %s ## %d ## %s ## %s]\n",
               players[i].id, players[i].player, players[i].height,
               players[i].weight, players[i].college, players[i].born,
               players[i].birth_city, players[i].birth_state);
    }
}
...
int main(void)
{
    ...
    Jogador atletasManipular[100];
    char id[100];
    int i = 0;
    int contadorEntrada = 0;

    // Leitura dos IDs a serem pesquisados
    while (strcmp(id, "FIM") != 0) {
        int idPesquisado = atoi(id);

        if (idPesquisado >= 0 && idPesquisado < 3922) {
            atletasManipular[i] = atleta[idPesquisado];
            i++;
        }
        contadorEntrada++;
        scanf("%s", id);
    ...
    {
        int menor = i;
        for (int j = (i + 1); j < contadorEntrada; j++)
        {
            if (strcmp(atletasManipular[menor].player, atletasManipular[j].player) > 0)
            {
                menor = j;
            }
        }
        Jogador temp = atletasManipular[i];
        atletasManipular[i] = atletasManipular[menor];
        atletasManipular[menor] = temp;
    }

    imprimir(atletasManipular, contadorEntrada);
