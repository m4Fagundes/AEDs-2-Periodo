...
void odenacaoIncercao(Jogador *jogadores, int numJogadores) {
    for (int i = 1; i < numJogadores; i++) {
        Jogador tmp = jogadores[i];
        int j = i - 1;
        while (j >= 0 && (tmp.anoNascimento < jogadores[j].anoNascimento || (tmp.anoNascimento == jogadores[j].anoNascimento && strcmp(tmp.nome, jogadores[j].nome) < 0))){
...
int main()
{
    ...
    odenacaoIncercao(atletasManipular, i2);
    //imprimir
